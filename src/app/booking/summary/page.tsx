...
export default function BookingSummaryPage() {
  ...
  useEffect(() => {
    if (user) {
      ...
      supabase
        ...
        .single()
        .then(({ data }) => {
          setDbBalance(data?.wallet_balance ?? 0);
        });
      fetchOffers();
    }
  }, [user?.id]);
  ...
  const walletBalance = dbBalance ?? user.walletBalance ?? 0;
  ...
  const removeWelcomeCouponAfterFirstBooking = async () => {
    try {
      await supabase
        .from('coupons')
        .update({ active: false })
    ...
    if (paymentMethod === 'wallet') {
      ...
      try {
        const { error: walletError } = await supabase
          .from('profiles')
          .update({ wallet_balance: walletBalance - finalAmount })
          .eq('id', user.id);
          if (walletError) throw walletError;

          setDbBalance(walletBalance - finalAmount);
        ...
        const result = await addBooking({
          ...
          if (result.success) {
            ...
            await refreshUser();
            removeWelcomeCouponAfterFirstBooking();
