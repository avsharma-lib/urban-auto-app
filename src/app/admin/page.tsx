...
import { LogOut, Check, RefreshCw, ChevronDown, ChevronUp, User, Phone, Mail, MapPin, Car, FileText, Calendar, Loader2, Copy, Users, CalendarDays, KeyRound, Eye, EyeOff, X, ShieldCheck, ShieldX, Ban, CheckCircle, IndianRupee, Wrench, AlertTriangle, Truck, Home, Search, Ticket, Plus, Trash2, ToggleLeft, ToggleRight } from 'lucide-react';
...
export default function AdminPanel() {
  ...
  const [userCouponDiscount, setUserCouponDiscount] = useState('');
  const [addingUserCoupon, setAddingUserCoupon] = useState<string | null>(null);
  ...
  const addCoupon = async () => {
    ...
    if (error) toast.error(error.message.includes('duplicate') ? 'Coupon code already exists' : 'Failed to add coupon');
    else {
  ...
  const toggleCoupon = async (id: string, active: boolean) => {
    const { error } = await supabase.from('coupons').update({ active: !active, updated_at: new Date().toISOString() }).eq('id', id);
  ...
  const addUserCoupon = async (userId: string) => {
    if (!userCouponCode.trim()) { toast.error('Enter coupon code'); return; }
    if (!userCouponDiscount || Number(userCouponDiscount) <= 0 || Number(userCouponDiscount) > 100) { toast.error('Enter valid discount (1-100%)'); return; }
    setAddingUserCoupon(userId);
    const { error } = await supabase.from('coupons').insert([{
      code: userCouponCode.trim().toUpperCase(),
      discount_percent: Number(userCouponDiscount),
      active: true,
      user_id: userId,
    }]);
    if (error) toast.error(error.message.includes('duplicate') ? 'Coupon code already exists' : 'Failed to add coupon');
    else {
      toast.success('User coupon created');
      setUserCouponCode('');
      setUserCouponDiscount('');
    }
    setAddingUserCoupon(null);
  };
  ...
  const updateWalletBalance = async (userId: string) => {
    ...
    try {
      ...
      setProfiles(prev => prev.map(p => p.id === userId ? { ...p, wallet_balance: newBalance } : p));
                ...
                <div className="p-4 cursor-pointer" onClick={() => toggleUserExpand(profile.id)}>
                  <div className="flex items-center gap-3">
                    ...
                    <div className="mt-3 space-y-1.5">
                      ...
                      <div className="flex items-center gap-2"><IndianRupee className="w-3.5 h-3.5 text-green-600" /><span className="text-xs font-semibold text-green-700">Wallet: ₹{(profile.wallet_balance ?? 0).toLocaleString('en-IN')}</span></div>
                    </div>
                </div>
                          ...
                          {!profile.blocked ? (
                            ...
                            <div className="mb-4 p-3 bg-green-50 border border-green-200 rounded-xl">
                              <h4 className="text-xs font-bold text-green-700 uppercase tracking-wider mb-2 flex items-center gap-1.5">
                                <IndianRupee className="w-3.5 h-3.5" /> Update Wallet Balance
                              </h4>
                              <p className="text-[10px] text-green-600 mb-2">Current: ₹{(profile.wallet_balance ?? 0).toLocaleString('en-IN')}</p>
                              <div className="flex gap-2">
                                <div className="flex-1 relative">
                                  <span className="absolute left-2.5 top-1/2 -translate-y-1/2 text-gray-400 text-xs">₹</span>
                                  <input
                                    type="number"
                                    value={walletUserId === profile.id ? walletAmount : ''}
                                    onChange={(e) => { setWalletUserId(profile.id); setWalletAmount(e.target.value); }}
                                    placeholder="Amount"
                                    min="1"
                                    className="w-full pl-6 pr-2 py-2 rounded-lg border border-green-200 bg-white text-xs outline-none focus:border-green-400"
                                    onClick={(e) => e.stopPropagation()}
                                  />
                                </div>
                                <button
                                  onClick={(e) => { e.stopPropagation(); updateWalletBalance(profile.id); }}
                                  disabled={walletLoading && walletUserId === profile.id}
                                  className="px-3 py-2 bg-green-600 text-white rounded-lg text-xs font-bold disabled:opacity-60 flex items-center gap-1"
                                >
                                  {walletLoading && walletUserId === profile.id ? <Loader2 className="w-3 h-3 animate-spin" /> : <Plus className="w-3 h-3" />}
                                  Add
                                </button>
                              </div>
                            </div>
                            ...
                            <h4 className="text-xs font-bold text-purple-700 uppercase tracking-wider mb-2 flex items-center gap-1.5">
                              <Ticket className="w-3.5 h-3.5" /> Create Coupon for {profile.full_name || 'User'}
                            </h4>
                            <div className="flex gap-2">
                              <input
                                type="text"
                                value={userCouponCode}
                                onChange={(e) => setUserCouponCode(e.target.value.toUpperCase())}
                                placeholder="Code"
                                className="flex-1 px-3 py-2 rounded-lg border border-purple-200 bg-white text-xs outline-none focus:border-purple-400 uppercase"
                                onClick={(e) => e.stopPropagation()}
                              />
                              <input
                                type="number"
                                value={userCouponDiscount}
                                onChange={(e) => setUserCouponDiscount(e.target.value)}
                                placeholder="%"
                                min="1"
                                max="100"
                                className="w-16 px-2 py-2 rounded-lg border border-purple-200 bg-white text-xs outline-none focus:border-purple-400 text-center"
                                onClick={(e) => e.stopPropagation()}
                              />
                              <button
                                onClick={(e) => { e.stopPropagation(); addUserCoupon(profile.id); }}
                                disabled={addingUserCoupon === profile.id}
                                className="px-3 py-2 bg-purple-600 text-white rounded-lg text-xs font-bold disabled:opacity-60 flex items-center gap-1"
                              >
                                {addingUserCoupon === profile.id ? <Loader2 className="w-3 h-3 animate-spin" /> : <Plus className="w-3 h-3" />}
                                Add
                              </button>
                            </div>
                          </div>
                                    ...
                                    <p className="text-[10px] text-gray-500 mt-0.5">{b.vehicle_type}{b.vehicle_number ? ` - ${b.vehicle_number}` : ''}</p>
                                      ...
                                      <div className="flex items-center gap-1.5 mt-0.5">
                                        <span className="text-[10px] font-semibold text-gray-700">
                                          ...
                                          <span className={`px-1.5 py-0.5 rounded-full text-[8px] font-bold uppercase ${(b as any).payment_status === 'paid' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-600'}`}>
                                            {(b as any).payment_status === 'paid' ? 'Paid' : 'Unpaid'}
                                          </span>
                                      </div>
                                  </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4">
              ...
              <div className="space-y-3">
                <input
                  ...
                  className="w-full px-4 py-3 rounded-xl border border-gray-200 bg-gray-50 text-sm outline-none focus:border-primary uppercase"
                />
                <div className="flex gap-3">
                  <div className="flex-1 relative">
                    <input
                      ...
                      min="1"
                      max="100"
                      className="w-full px-4 py-3 rounded-xl border border-gray-200 bg-gray-50 text-sm outline-none focus:border-primary pr-8"
                    />
                  ...
                  <button
                    ...
                    className="px-6 py-3 bg-primary text-white rounded-xl text-sm font-bold disabled:opacity-60 flex items-center gap-2"
                  >
                  ...
                  <div className="p-4 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        className={`p-2 rounded-lg transition-colors ${coupon.active ? 'bg-green-100 hover:bg-green-200' : 'bg-gray-100 hover:bg-gray-200'}`}
                      >
